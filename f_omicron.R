...
# parameters
beta <- read.xlsx('Beta_28.xlsx', sheet = 3, colNames = F)
beta <- as.matrix(beta) # matrix of transmission rates
kappa1 <- 0.63
kappa2 <- 0.35
HR <- rep(c(1, 0.6992, 0.5531, 0.3665), 7)
mu <- rep(c(0.55, 0.55, 0.55, 0.55), 7)
omega <- 1/3
omegap <- 1/2 # rate at which individuals move from the exposed to the pre-symptom classes
omegapp <- 1/1
...
beta <- read.xlsx('Beta_28_cut_f.xlsx', sheet = 3, colNames = F)
beta <- as.matrix(beta) 
parms <- list(kappa1 = kappa1, kappa2 = kappa2, HR = HR, beta=beta, mu=mu, omega=omega, omegap = omegap,
              omegapp = omegapp, gamma=gamma, gammap = gammap, f = f)
...
beta <- read.xlsx('Beta_28_cut_s.xlsx', sheet = 3, colNames = F)
beta <- as.matrix(beta) 
parms <- list(kappa1 = kappa1, kappa2 = kappa2, HR = HR, beta=beta, mu=mu, omega=omega, omegap = omegap,
              omegapp = omegapp, gamma=gamma, gammap = gammap, f = f)
...
#         ggplot(aes(x = time, y= value, color = vac_group)) +
#         
#         geom_line() +
#         
#         scale_y_continuous(
...
# 
# 
# ggsave('test_omicron_nc.pdf', height = 12, width = 8, dpi = 300)
