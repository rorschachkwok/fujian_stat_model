...
# 
# for (i in 1:m) {
#         tmp <- data.frame(group = c(seq(i, 5*m+i, 28)),
#                          age = age_id[[i]],
#                          vac = vac_id[[i]])
#         group_id <- bind_rows(group_id, tmp)
# 
# }
# 
# group_id <- group_id %>%
#         mutate(id = paste0('age', age, '_vac', vac)) %>%
#         select(group, id)
...
ggsave('delta_I_A.pdf', height = 12, width = 8, dpi = 300)
